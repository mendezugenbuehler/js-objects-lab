...
for (let i = 0; i < pokemon.length; i++) {
    ...
    if (
      currentPokemon.type.includes("water") || currentPokemon.hp > 46 
    ) {
...
for (let i = 0; i < game.party.length; i++) {
    const number = game.party[i].number;

    let newPokemon;
    if (number === 1) {
        newPokemon = pokemon[1]; 
    } else if (number === 2) {
        newPokemon = pokemon[2]; 
    } else if (number === 3) {
        newPokemon = pokemon[2]; 
    } else if (number === 5) {
        newPokemon = pokemon[5]; 
    }
    
    if (newPokemon) {
        game.party.splice(i, 1, { ...newPokemon });
    }
}
...
const partyCount = game.party.length;
